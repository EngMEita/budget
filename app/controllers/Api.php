...
class Api extends CI_Controller {
    ...
    public function index ()
    {
        ...
        $to                 = strtotime ( 'now' ) ;
        $data['res']        = $this->m->getQuery ( 'SELECT * FROM `ledger` WHERE `rec_time` >= '.$from.' AND `rec_time` <= '.$to.' ORDER BY `rec_time` DESC, `rec_id` DESC' );
    ...
    public function budgets ( $mod = 'list', $budget_id = null )
    {
        ...
        switch ( $mod )
        {
            ...
            case 'list':
            default:
                $data['res']        = $this->m->getRes ( array ( 'table' => 'budgets' ), 'object' ) ;
                $data['page_title'] = 'الميزانيات' ;
            break;
        }
        $data['mod']                = $mod;
    ...
    public function ledger ( $str = null )
    {
        $data = $this->g ;
        $dt['table']        = "ledger" ; 
        $dt['cond']         = ! is_null ( $str ) ? str2array ( $str ) : null ;
        $this->session->set_flashdata ( 'ledger_cond', $dt ) ;
        $dt['sort']         = array ( 'rec_time' => 'asc', 'rec_id' => 'asc' ) ;
        $data['res']        = $this->m->getRes ( $dt ) ;
    ...
    {
        if ( isset ( $_POST['money'] ) && intval ( $_POST['money'] ) > 0 )
        {
    ...
    public function save ()
    {
        ...
        if ( isset ( $_POST['table'] ) )
        {
            if ( isset ( $_POST['frm_data'] ) && is_array ( $_POST['frm_data'] ) && ! empty ( $_POST['frm_data'] ) )
            {
                ...
                if ( $this->m->setRec ( $table, $data, $id_fld, $id_vlu ) ) $data['message'] = "done";
                else $data['message'] = "error 3";
            } 
            else $data['message'] = "error 2";
        }
        else $data['message'] = "error1"; 
